...
select distinct s.КодовыйНомерПредмета, s.НазваниеПремета from subject s
    inner join teacher_student_group tsg on tsg.КодовыйНомерПредмета = s.КодовыйНомерПредмета
    inner join teacher t on tsg.ЛичныйНомер = t.ЛичныйНомер
...
select tsg.КодовыйНомерГруппы from teacher_student_group tsg
    inner join teacher on teacher.ЛичныйНомер = tsg.ЛичныйНомер
...
select Фамилия from teacher t
    inner join teacher_student_group tsg on  t.ЛичныйНомер = tsg.ЛичныйНомер
...
select s.НазваниеПремета, sg.НазваниеГруппы from teacher_student_group tsg
    inner join subject s on tsg.КодовыйНомерПредмета = s.КодовыйНомерПредмета
...
select sg1.КодовыйНомерГруппы, sg2.КодовыйНомерГруппы from student_group sg1
    inner join student_group sg2 on sg1.Специальность = sg2.Специальность
...
select Фамилия from teacher t
    inner join teacher_student_group tsp on t.ЛичныйНомер = tsp.ЛичныйНомер
...
select distinct sg.КодовыйНомерГруппы from student_group sg
    inner join teacher_student_group tsg on sg.КодовыйНомерГруппы = tsg.КодовыйНомерГруппы
    inner join (select ЛичныйНомер, unnest(string_to_array(Специальность, ', ')) Специальность from teacher) t
...
select s.КодовыйНомерПредмета from subject s
    inner join teacher_student_group tsg on s.КодовыйНомерПредмета = tsg.КодовыйНомерПредмета
    inner join student_group sg on tsg.КодовыйНомерГруппы = sg.КодовыйНомерГруппы
